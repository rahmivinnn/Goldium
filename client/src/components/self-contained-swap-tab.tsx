...
export function SelfContainedSwapTab() {
  ...
  const displayRate = fromToken === 'SOL' 
    ? `1 SOL = ${SOL_TO_GOLD_RATE.toLocaleString()} GOLD`
  ...
  const getTokenBalance = (token: 'SOL' | 'GOLD') => {
    if (token === 'SOL') {
      // Use instant balance for immediate updates when switching wallets
      return externalWallet.connected ? instantBalance.balance : balances.sol;
    }
              ...
              >
                {fromToken === 'SOL' ? (
                  ...
                  <div className="w-6 h-6 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded flex items-center justify-center">
                    ...
                    />
                    <div className="text-gold-primary text-sm font-bold hidden">⚡</div>
                  </div>
                )}
