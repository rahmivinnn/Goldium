...
export function GoldPriceWidget() {
  const [priceData, setPriceData] = useState<PriceData>({
    goldPriceSOL: 0.00004422,
    goldPriceUSD: 0.007562,
    solPriceUSD: 172.03,
    change24h: -4.28,
    volume24h: 45900
  });
  ...
  const fetchGoldPrice = async () => {
    ...
    try {
      ...
      const mockPrice = {
        goldPriceSOL: 0.00004422 + (Math.random() - 0.5) * 0.000005,
        goldPriceUSD: 0.007562 + (Math.random() - 0.5) * 0.001,
        solPriceUSD: 172.03 + (Math.random() - 0.5) * 5,
        change24h: -4.28 + (Math.random() - 0.5) * 2,
        volume24h: 45900 + Math.random() * 5000
      };
