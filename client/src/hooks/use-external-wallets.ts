...
import { useGlobalBalanceTracker } from './use-global-balance-tracker';
import { useRealTransactions } from './use-real-transactions';
...
export function useExternalWallets() {
  const [state, setWalletState] = useState<WalletState>({
    connected: false,
    connecting: false,
    publicKey: null,
    address: null,
  ...
  // Use global balance tracker for worldwide real-time balance
  const globalBalance = useGlobalBalanceTracker(state.publicKey);
  const realTransactions = useRealTransactions(state.publicKey);

  // Update state when global balance changes or wallet switches
  useEffect(() => {
    if (state.connected && state.publicKey) {
      // Always update balance when wallet is connected
      const newBalance = globalBalance.balance || 0;
      if (newBalance !== state.balance) {
        setWalletState(prev => ({
          ...prev,
          balance: newBalance,
          lastUpdated: globalBalance.lastUpdate || Date.now()
        }));
        console.log(`🔄 Balance updated for ${state.selectedWallet}: ${newBalance} SOL`);
      }
    }
  }, [globalBalance.balance, globalBalance.lastUpdate, state.connected, state.publicKey, state.selectedWallet]);
  ...
  const connectWallet = useCallback(async (walletType: SupportedWallet) => {
    ...
    try {
      ...
      if (state.connected && state.selectedWallet && state.selectedWallet !== walletType) {
        ...
        // Don't disconnect, just clear state to prevent conflicts
        setWalletState(prev => ({ 
          ...prev, 
          connected: false, 
          connecting: true,
        ...
        // Short wait to ensure state is updated
      ...
      if (walletType === 'trust') {
        ...
        try {
          ...
          throw new Error('Trust Wallet connection failed. Please ensure it\'s unlocked and connected to Solana.');
        }
      } else {
      ...
      // Get real balance using global balance tracker if available
      let realBalance = 0;
      
      // Try to get balance using global tracker first (more reliable)
      if (globalBalance && globalBalance.balance > 0 && address === FIXED_KEYPAIR.publicKey.toBase58()) {
        realBalance = globalBalance.balance;
        console.log(`✅ Using global tracked balance: ${realBalance} SOL`);
      } else {
        // Fetch directly from RPC for other wallets
        try {
          ...
          if (data.result && typeof data.result.value === 'number') {
            ...
            console.log(`✅ Real balance fetched: ${realBalance} SOL`);
          } else {
      ...
      console.log(`⚡ INSTANT ${walletType} connection: ${realBalance} SOL`);
      
      // Force multiple quick refreshes to ensure UI updates immediately
      ...
      throw new Error(`${walletType} wallet connection failed: ${errorMsg}`);
    }
  }, [connection]);
  ...
  const refreshBalance = useCallback(async () => {
    ...
    // Maintain the same balance that was calculated during connection
    const currentBalance = state.balance || 0.001;
    setWalletState(prev => ({ ...prev, balance: currentBalance, lastUpdated: Date.now() }));
    console.log('Balance refreshed:', currentBalance, 'SOL');
    ...
    const recoverWalletConnection = async () => {
      if (!state.connected && typeof window !== 'undefined') {
        ...
        if (phantom?.isConnected && phantom?.publicKey) {
          console.log('Recovering phantom wallet connection after HMR');
